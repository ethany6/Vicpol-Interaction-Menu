...
RegisterNetEvent('InteractionMenu:Cuff')
AddEventHandler('InteractionMenu:Cuff', function()
...
local Drag = false
local OfficerDrag = -1
RegisterNetEvent('InteractionMenu:Drag')
AddEventHandler('InteractionMenu:Drag', function(ID)
	Drag = not Drag
	OfficerDrag = ID
...
RegisterNetEvent('InteractionMenu:Seat')
AddEventHandler('InteractionMenu:Seat', function(Veh)
...
RegisterNetEvent('InteractionMenu:Unseat')
AddEventHandler('InteractionMenu:Unseat', function(ID)
...
local SpawnedSpikes = {}
RegisterNetEvent('InteractionMenu:Spikes-SpawnSpikes')
AddEventHandler('InteractionMenu:Spikes-SpawnSpikes', function(Length)
...
RegisterNetEvent('InteractionMenu:Spikes-DeleteSpikes')
AddEventHandler('InteractionMenu:Spikes-DeleteSpikes', function()
...
--Backup
RegisterNetEvent('InteractionMenu:CallBackup')
AddEventHandler('InteractionMenu:CallBackup', function(Code, StreetName, Coords)
    if VICPOLRestrict() then
        ...
        if Code == 1 then
            Notify('An officer is requesting ~g~Priority 1 ~w~backup at ~b~' .. StreetName)
            CreateBlip(Coords.x, Coords.y, Coords.z, 'Priority 1 Backup Requested', 56, 0.8, 2)
        elseif Code == 2 then
            Notify('An officer is requesting ~y~Priority 2 ~w~backup at ~b~' .. StreetName)
            CreateBlip(Coords.x, Coords.y, Coords.z, 'Priority 2 Backup Requested', 56, 0.8, 17)
        elseif Code == 3 then
            Notify('An officer is requesting ~r~Priority 3 ~w~backup at ~b~' .. StreetName)
            CreateBlip(Coords.x, Coords.y, Coords.z, 'Priority 3 Backup Requested', 56, 1.0, 49)

        elseif Code == 'panic' then
            Notify('An officer has pressed their ~r~Panic Button ~w~at ~b~' .. StreetName)
            CreateBlip(Coords.x, Coords.y, Coords.z, 'Panic Button Pressed respond SIGNAL 1', 103, 1.2, 49)
        end
    end
end)
...
EarlyRelease = false
OriginalJailTime = 0
RegisterNetEvent('InteractionMenu:JailPlayer')
AddEventHandler('InteractionMenu:JailPlayer', function(JailTime)
    ...
    if DoesEntityExist(Ped) then
        Citizen.CreateThread(function()
            ...
            if EarlyRelease then
                TriggerServerEvent('InteractionMenu:GlobalChat', {86, 96, 252}, 'Judge', GetPlayerName(PlayerId()) .. ' was released from Jail on Parole')
            else
                TriggerServerEvent('InteractionMenu:GlobalChat', {86, 96, 252}, 'Judge', GetPlayerName(PlayerId()) .. ' was released from Jail after ' .. OriginalJailTime .. ' months(s).')
            end
...
RegisterNetEvent('InteractionMenu:UnjailPlayer')
AddEventHandler('InteractionMenu:UnjailPlayer', function()
    EarlyRelease = true
end)



--Toggle VICPOL Weapons
...
--Civilian Adverts
RegisterNetEvent('InteractionMenu:SyncAds')
AddEventHandler('InteractionMenu:SyncAds',function(Text, Name, Loc, File, ID)
...
--Inventory
RegisterNetEvent('InteractionMenu:InventoryResult')
AddEventHandler('InteractionMenu:InventoryResult', function(Inventory)
    Citizen.Wait(5000)
...
--BAC
RegisterNetEvent('InteractionMenu:BACResult')
AddEventHandler('InteractionMenu:BACResult', function(BACLevel)
    Citizen.Wait(5000)
...
RegisterNetEvent('InteractionMenu:HospitalizePlayer')
AddEventHandler('InteractionMenu:HospitalizePlayer', function(HospitalTime, HospitalLocation)
    if CurrentlyHospitaled then
        return
    end
    if CurrentlyJailed then
    ...
    if DoesEntityExist(Ped) then
        Citizen.CreateThread(function()
            ...
            if EarlyDischarge then
                TriggerServerEvent('InteractionMenu:GlobalChat', {86, 96, 252}, 'Doctor', GetPlayerName(PlayerId()) .. ' was discharged from Hospital early')
            else
                TriggerServerEvent('InteractionMenu:GlobalChat', {86, 96, 252}, 'Doctor', GetPlayerName(PlayerId()) .. ' was discharged from Hospital after ' .. OriginalHospitalTime .. ' months(s).')
            end
...
RegisterNetEvent('InteractionMenu:UnhospitalizePlayer')
AddEventHandler('InteractionMenu:UnhospitalizePlayer', function()
    EarlyDischarge = true
end)
...
Citizen.CreateThread(function()
    if Config.DisplayStationBlips then
        ...
        for _, Station in pairs(Config.VICPOLStations) do
            CreateBlip(Station.coords.x, Station.coords.y, Station.coords.z, 'VICPOL Station', 38, 60)
        end
        for _, Station in pairs(Config.FrvStations) do
            CreateBlip(Station.coords.x, Station.coords.y, Station.coords.z, 'FRV Station', 1, 60)
        end
...
--Permissions
VICPOLAce = false
TriggerServerEvent('InteractionMenu:VICPOLPerms')
RegisterNetEvent('InteractionMenu:VICPOLPermsResult')
AddEventHandler('InteractionMenu:VICPOLPermsResult', function(Allowed)
    if Allowed then
        VICPOLAce = true
    else
        VICPOLAce = false
...
FrvAce = false
TriggerServerEvent('InteractionMenu:FrvPerms')
RegisterNetEvent('InteractionMenu:FrvPermsResult')
AddEventHandler('InteractionMenu:FrvPermsResult', function(Allowed)
    if Allowed then
        FrvAce = true
    else
        FrvAce = false
...
UnjailAllowed = false
TriggerServerEvent('InteractionMenu:UnjailPerms')
RegisterNetEvent('InteractionMenu:UnjailPermsResult')
AddEventHandler('InteractionMenu:UnjailPermsResult', function(Allowed)
    if Allowed then
        UnjailAllowed = true
    else
        UnjailAllowed = false
...
TriggerServerEvent('InteractionMenu:UnhospitalPerms')
RegisterNetEvent('InteractionMenu:UnhospitalPermsResult')
AddEventHandler('InteractionMenu:UnhospitalPermsResult', function(Allowed)
    if Allowed then
...
Citizen.CreateThread(function()
    ...
    TriggerEvent('chat:addSuggestion', '/loadout', 'Equips VICPOL Weapon Loadout')
    ...
    if Config.VICPOLAccess == 3 or Config.FrvAccess == 3 then
        if Config.OndutyPSWDActive then
            TriggerEvent('chat:addSuggestion', '/onduty', 'Enable VICPOL/Frv Menu', {{name = 'Department', help = 'VICPOL or Frv'}, {name = 'Password', help = 'Onduty Password'}})
        else
            TriggerEvent('chat:addSuggestion', '/onduty', 'Enable VICPOL/Frv Menu', {{name = 'Department', help = 'VICPOL or Frv'}})
        end
    else
    ...
    if Config.VICPOLAccess == 3 or Config.FrvAccess == 3 then
        if Config.OndutyPSWDActive then
            if args[2] == Config.OndutyPSWD then
                ...
                if Department == 'vicpol' then
                    VICPOLOnduty = not VICPOLOnduty
                    if VICPOLOnduty then
                        Notify('~g~You are onduty as an VICPOL')
                    else
                        Notify('~o~You are no longer onduty as an vicpol')
                    end
                elseif Department == 'frv' then
                    FrvOnduty = not FrvOnduty
                    if FrvOnduty == true then
                        Notify('~g~You are onduty as an Frvfighter')
                    else
                        Notify('~o~You are no longer onduty as an Frvfighter')
                    end
                else
...
RegisterCommand('cuff', function(source, args, rawCommand)
    if VICPOLRestrict() or FrvRestrict() then
        if args[1] ~= nil then
            ...
            TriggerServerEvent('InteractionMenu:CuffNear', GetClosestPlayer())
        end
    else
...
RegisterCommand('drag', function(source, args, rawCommand)
    if VICPOLRestrict() or FrvRestrict() then
        if args[1] ~= nil then
            ...
            if Config.CommandDistanceChecked then
                if GetDistance(source) < Config.CommandDistance then
                    TriggerServerEvent('InteractionMenu:DragNear', ID)
                else
                    Notify('~r~That player is too far away')
                end
            else
            ...
            TriggerServerEvent('InteractionMenu:DragNear', GetClosestPlayer())
        end
    else
        Notify('~r~Insufficient Permissions')
    end
