    ...
    if Config.Radar ~= 0 then
        if IsPedInAnyVehicle(GetPlayerPed(-1)) then
            if GetVehicleClass(GetVehiclePedIsIn(GetPlayerPed(-1))) == 18 then
                ...
                Notify('~o~You need to be in a VICPOL vehicle')
            end
        else
...
--Menu Restrictions
function VICPOLRestrict()
    if Config.VICPOLAccess == 0 then
        return false
    elseif Config.VICPOLAccess == 1 then
        return true
    elseif Config.VICPOLAccess == 2 then
        local Ped = GetEntityModel(GetPlayerPed(-1))

        for _, VICPOLPeds in pairs(Config.VICPOLUniforms) do
            local AllowedPed = GetHashKey(VICPOLPeds.spawncode)

            if Ped == AllowedPed then
                return true
            end
        end
    elseif Config.VICPOLAccess == 3 then
        return VICPOLOnduty
    elseif Config.VICPOLAccess == 4 then
        return VICPOLAce
    else
        return true
    end
...
function FrvRestrict()
    if Config.FrvAccess == 0 then
        return false
    elseif Config.FrvAccess == 1 then
        return true
    elseif Config.FrvAccess == 2 then
        local Ped = GetEntityModel(GetPlayerPed(-1))

        for _, FrvPeds in pairs(Config.FrvUniforms) do
            local AllowedPed = GetHashKey(FrvPeds.spawncode)

            if Ped == AllowedPed then
                return true
            end
        end
    elseif Config.FrvAccess == 3 then
        return FrvOnduty
    elseif Config.FrvAccess == 4 then
        return FrvAce
    else
        return true
    end
